...
async function proposeMeetup() {
  if (window.__DEMO__) {
    ...
    window.__CAL__.addEvent(newEvent);
    scrollToFirstEvent([{
      start: formatScheduleXDate(start),
      end: formatScheduleXDate(end)
    }]);
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (eventPopover) {
    ...
    if (window.__DEMO__) {
      ...
      try {
        const r = await fetch(`/api/groups/${window.__GROUP_ID__}/events/delete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function loadCalendars() {
    if (isDemo) {
      ...
      demoMembers.forEach((m) => {
        ...
        demoCalendars[key] = {
          ...
          lightColors: { main: m.color, container: "#eef2ff", onContainer: "#1f2937" },
          darkColors: { main: m.color, container: "#0b1220", onContainer: "#e2e8f0" }
        };
      });
    ...
    members.forEach((m) => {
      ...
      memberNameById[m.user_id] = m.display_name || m.name || m.email || `Member ${m.user_id}`;
      calendars[key] = {
        colorName: key.toLowerCase(),
  ...
  const views = [viewWeek, viewDay, viewMonth, viewList].filter(Boolean);
  if (!views.length) {
  ...
  function scrollToFirstEvent(events) {
    ...
    const sorted = events.slice().sort((a, b) => {
      const aHour = a.start?.hour ?? 0;
      const aMinute = a.start?.minute ?? 0;
      const bHour = b.start?.hour ?? 0;
      const bMinute = b.start?.minute ?? 0;
  ...
  calendarInstance = createCalendar({
    ...
    callbacks: {
      fetchEvents: async function (range) {
        calendarRange = range;
        if (isDemo) {
          scrollToFirstEvent(demoEventsLimited);
